...
filename = 'early_fusion_predictions'
y_pred, y_true = pickle.load(open(filename + '.sav', 'rb'))

#filename = 'late_fusion_predictions'				 # saved only predictions for late fusion
#y_pred = pickle.load(open(filename + '.sav', 'rb')) # so only uncomment this two lines and keep the above lines uncommented
